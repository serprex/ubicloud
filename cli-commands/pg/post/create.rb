...
class UbiCli
  on("pg").run_on("create") do
    ...
    options("ubi pg location/pg-name create [options]", key: :pg_create) do
      ...
      on("-w", "--wal-drive-size-gib=size", Option::POSTGRES_WAL_DRIVE_SIZE_OPTIONS.map(&:to_s), "WAL drive size GB (nvme WAL drive only)")
      on("--network-volume-iops=iops", "provisioned IOPS for the network data volume")
      on("--network-volume-throughput-mibps=mibps", "provisioned throughput MiB/s for the network data volume")
      on("--wal-drive-iops=iops", "provisioned IOPS for a network WAL drive")
      on("--wal-drive-throughput-mibps=mibps", "provisioned throughput MiB/s for a network WAL drive")
    ...
    help_option_values("WAL Drive Size GB:", Option::POSTGRES_WAL_DRIVE_SIZE_OPTIONS)
