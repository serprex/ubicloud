...
class Prog::Postgres::PostgresResourceNexus < Prog::Base
  ...
  def self.assemble(project_id:, location_id:, name:, target_vm_size:, target_storage_size_gib:,
    ...
    DB.transaction do
      ...
      # Ignore configuration when the selected storage has no matching network volume.
      ...
      postgres_resource = PostgresResource.create_with_id(postgres_resource_id,
        ...
        superuser_password:, ha_type:, storage_type:, network_volume_type:, wal_drive_type:, wal_drive_size_gib:,
      ...
      # Record requested configuration on the first server's volumes.
      storage_config = {
