...
class Prog::Postgres::PostgresServerNexus < Prog::Base
  ...
  def self.assemble(resource_id:, timeline_id:, timeline_access:, is_representative: false, exclude_host_ids: [], exclude_availability_zones: [], availability_zone: nil, exclude_data_centers: [], storage_config: nil)
    DB.transaction do
      ...
      if postgres_resource.storage_type == PostgresResource::StorageType::NETWORK_CACHE
        ...
        # forfeiting stop/start: pg_wal dies with the instance store and
        # postgres cannot start without it. other types get a dedicated network
  ...
  # Data on a persistent network volume fronted by local NVMe write-through
  # bcache. setup-bcache assembles /dev/bcache0 and installs a boot-time unit to
  # re-assemble across reboot/stop-start, though stop/start only brings postgres
  # back when WAL is on a network volume (see bcache-assemble). fstab uses
  # nofail since /dev/bcache0 only exists after that unit runs.
  label def setup_bcache
    ...
    when "Failed", "NotStarted"
      data_device, wal_device = vm.vm_storage_volumes.select(&:provider_volume_id).sort_by(&:disk_index).map(&:device_path)
      wal_spec = wal_device || resource.wal_drive_size_gib.to_s
      vm.sshable.d_run("setup_bcache", "sudo", "postgres/bin/setup-bcache", data_device, wal_spec, postgres_server.instance_store_device_glob)
    end

    nap 5
  end

  # Doubling keeps volume modifications rare: providers rate limit them, and
  # logical slots can retain WAL much faster than the limit allows
  label def resize_wal_volume
    wal_volume = postgres_server.wal_volume
    size_gib = wal_volume.size_gib * 2
    hop_wait unless postgres_server.grow_wal_volume(size_gib)
    wal_volume.update(size_gib:)
    hop_wait_wal_volume_resized
  end

  label def wait_wal_volume_resized
    device_path = postgres_server.wal_volume.device_path
