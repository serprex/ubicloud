...
module Validation
  ...
  # Validate volume configuration before provisioning so callers receive actionable errors.
  def self.validate_network_volume_config(volume_type, size_gib, iops, throughput_mibps)
    limits = Option::NETWORK_VOLUME_LIMITS[volume_type]
    ...
    if throughput_mibps
      unless limits.throughput_mibps
    ...
    return unless limits.throughput_mibps

    # Ratio limits bind the pair the volume is actually created with, so an
    # unrequested side is checked at the type's baseline rather than skipped.
    # Otherwise a lone IOPS or throughput value passes here and the provider
