...
RSpec.describe PostgresSetup do
  ...
  describe "GO_SERVICES" do
    it "sum of GOMEMLIMIT values stays within the slice MemoryHigh" do
      ...
      expect(sum).to be <= 2 * 1024**3 # MemoryHigh=2G on go_services.slice
    end
  end
  ...
  describe "#configure_service_slice" do
    it "writes slice + drop-ins, reloads, sets slice property, restarts only services not yet in the slice" do
      expect(pg_setup).to receive(:safe_write_to_file).with("/etc/systemd/system/go_services.slice", <<~SLICE)
        [Slice]
        MemoryHigh=2G
        MemoryMax=2560M
      SLICE
      ...
      expect(pg_setup).to receive(:r).with("systemctl set-property go_services.slice MemoryHigh=2G MemoryMax=2560M")

      # First two services already in go_services.slice -> skip restart.
      # Last two still in system.slice / missing -> try-restart.
      slices = ["go_services.slice", "go_services.slice", "system.slice", ""]
      PostgresSetup::GO_SERVICES.each_key.with_index do |svc, i|
        ...
        if slices[i] != "go_services.slice"
          expect(pg_setup).to receive(:r).with("systemctl try-restart #{svc}.service", expect: [0, 1])
        end
      end
